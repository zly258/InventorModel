...
internal sealed class AiSettingsWindow : Window
{
    ...
    private UIElement BuildLayout()
    {
        ...
        intro.Children.Add(new TextBlock
        {
            Text = "使用 OpenAI Compatible 接口。配置会保存在当前用户目录，仅供 InventorModel 使用。",
        ...
        form.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(118) });
        ...
        AddRow(form, 0, "接口地址", _baseUrl, "示例：http://127.0.0.1:11434/v1 或兼容 /v1 的服务地址");
        AddRow(form, 1, "API Key", _apiKey, "本地服务可留空；云端服务按提供商要求填写");
        AddRow(form, 2, "模型", _model, "填写接口实际暴露的模型名称");
        AddRow(form, 3, "温度", _temperature, "范围 0–2；建模任务建议使用较低温度");
    ...
    private static void AddRow(Grid grid, int row, string label, Control control, string hint)
    {
    ...
    private void Save_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show(this, "模型名称不能为空。", "InventorModel", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
        ...
        {
            MessageBox.Show(this, "接口地址必须是有效的 http/https 地址。", "InventorModel", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
        ...
        if (!double.TryParse(
            ...
            temperature > 2)
        {
            MessageBox.Show(this, "温度必须在 0 到 2 之间。", "InventorModel", MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
